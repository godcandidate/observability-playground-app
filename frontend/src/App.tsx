    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-stretch">
        ...
        <div className="flex-1 p-6 flex flex-col items-center justify-center space-y-6 bg-gray-50">
          ...
          >
            <Icon
              className={`w-24 h-24 ${currentStatus.iconColor} transition-colors duration-500`}
            />
          </motion.div>
          ...
          {isSimulating && (
            ...
            >
              ...
              >
                {status === "good"
                  ? "Simulating optimal resource patterns"
                  : status === "warning"
                  ? "Testing warning threshold responses"
                  : status === "critical"
                  ? "Validating critical state handling"
...
function MetricsPanel() {
  const [metrics, setMetrics] = useState<
    Array<{ name: string; value: number; unit: string; timestamp: string }>
  >([]);
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                value={services}
                onChange={(e) =>
                  setServices(Math.max(1, parseInt(e.target.value)))
                }
                min="1"
                className="w-full p-2 border border-gray-300 rounded-md"
              />
            ...
            <motion.button
              ...
              className="w-full py-3 px-4 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-lg font-medium shadow-md hover:shadow-lg transition-all"
...
function Dashboard() {
  ...
  const handleSimulation = async (type: TabType) => {
    ...
    try {
      ...
      if (response?.status) {
        setResourceStates((prev) => ({
          ...
          [type]: {
            ...prev[type],
            status: response.status as ResourceState["status"],
          },
        }));
      }
  ...
  const handlePercentageChange = (type: TabType, value: number) => {
    setResourceStates((prev) => ({
      ...prev,
  ...
  const handleDurationChange = (type: TabType, value: string) => {
    setResourceStates((prev) => ({
      ...prev,
  ...
  const tabs = [
    { key: "memory" as TabType, label: "Memory Tests", icon: MemoryStick },
    { key: "cpu" as TabType, label: "CPU Tests", icon: Cpu },
    { key: "disk" as TabType, label: "Disk Tests", icon: HardDrive },
    { key: "logs" as TabType, label: "Log Patterns", icon: ScrollText },
    { key: "metrics" as TabType, label: "Metric Patterns", icon: Activity },
    { key: "traces" as TabType, label: "Trace Patterns", icon: Network },
  ];
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 lg:px-8 min-h-[calc(100vh-12rem)]">
        {activeTab === "memory" && (
          <ResourceCard
            ...
            onPercentageChange={(value) =>
              handlePercentageChange("memory", value)
            }
        ...
        {activeTab === "disk" && (
          <ResourceCard
            ...
            onPercentageChange={(value) =>
              handlePercentageChange("disk", value)
            }
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-lg text-gray-600 mb-8">
              Experience a comprehensive playground for testing and exploring
              various observability features. Monitor CPU usage, memory
              allocation, generate logs, emit custom metrics, and simulate
              distributed tracing.
            </p>
            <button
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-4">
              <div className="bg-white p-6 rounded-xl shadow-lg">
                ...
                <p className="text-sm text-gray-600">
                  Simulate and monitor CPU-intensive tasks
                </p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div className="space-y-4 mt-8">
              ...
              <div className="bg-white p-6 rounded-xl shadow-lg">
                ...
                <p className="text-sm text-gray-600">
                  Simulate distributed system traces
                </p>
              </div>
            </div>
          </div>
...
function CountdownTimer({ duration }: { duration: string }) {
  ...
  return <div className="font-mono text-sm text-gray-600">{timeLeft}</div>;
}
